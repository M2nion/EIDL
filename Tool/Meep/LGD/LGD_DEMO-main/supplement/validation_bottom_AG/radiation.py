...
sources = [
    mp.Source(
        src=mp.GaussianSource(fcen, fwidth=df), center=mp.Vector3(0,-sxy/2), component=src_cmpt
    )
]
...
geometry = [
    mp.Block(
        center=mp.Vector3(0, -sxy/2 - dpml/2, 0),
        size=mp.Vector3(sxy, dpml, 0),
        material=ALU,
    ),
]
...
nearfield_box = sim.add_near2far(
    ...
    mp.Near2FarRegion(center=mp.Vector3(+0.5 * sxy,), size=mp.Vector3(0, sxy)),
    mp.Near2FarRegion(
        center=mp.Vector3(-0.5 * sxy,), size=mp.Vector3(0, sxy), weight=-1
    ),
)
...
flux_box = sim.add_flux(
    ...
    mp.FluxRegion(center=mp.Vector3(+0.5 * sxy,), size=mp.Vector3(0, sxy)),
    mp.FluxRegion(center=mp.Vector3(-0.5 * sxy,), size=mp.Vector3(0, sxy), weight=-1),
)
