...
cell = mp.Vector3(sxy + 2 * dpml, sxy + 1 * dpml)
...
sources = [
    mp.Source(
        src=mp.GaussianSource(fcen, fwidth=df), center=mp.Vector3(0,-sxy/2- dpml/2 + 1/resolution), component=src_cmpt
    )
]
...
nearfield_box = sim.add_near2far(
    ...
    0,
    1,
    mp.Near2FarRegion(center=mp.Vector3(0, +0.5 * sxy - dpml/2), size=mp.Vector3(sxy, 0)),
    mp.Near2FarRegion(
        center=mp.Vector3(0, -0.5 * sxy- dpml/2), size=mp.Vector3(sxy, 0), weight=-1
    ),
    mp.Near2FarRegion(center=mp.Vector3(+0.5 * sxy, - dpml/2), size=mp.Vector3(0, sxy)),
    mp.Near2FarRegion(
        center=mp.Vector3(-0.5 * sxy, - dpml/2), size=mp.Vector3(0, sxy), weight=-1
    ),
)
...
flux_box = sim.add_flux(
    ...
    0,
    1,
    mp.FluxRegion(center=mp.Vector3(0, +0.5 * sxy - dpml/2), size=mp.Vector3(sxy, 0)),
    mp.FluxRegion(center=mp.Vector3(0, -0.5 * sxy - dpml/2), size=mp.Vector3(sxy, 0), weight=-1),
    mp.FluxRegion(center=mp.Vector3(+0.5 * sxy, - dpml/2), size=mp.Vector3(0, sxy)),
    mp.FluxRegion(center=mp.Vector3(-0.5 * sxy, - dpml/2), size=mp.Vector3(0, sxy), weight=-1),
)
